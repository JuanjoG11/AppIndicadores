...
export const useKPIs = (currentUser, activeCompany, onToast) => {
    ...
    const applyKPIUpdate = useCallback((kpiId, newData, shouldPersist = true, forceHistorical = false) => {
        ...
        setKpiData(prevData => {
            ...
            // Periodo reportable: mes/semana/quincena anterior
            ...
            // Para granulares: solo aceptar el periodo reportable exacto (semana/quincena anterior)
            // Para no granulares: aceptar el mes reportable
            const isStrictCurrent = recordPeriodIndex === currentReportablePeriod;
            ...
            // Para SEMANAL y QUINCENAL: aceptar cualquier periodo dentro del mes reportable
            // Para DIARIO: solo el día actual
            const isSameReportableMonth = currentMonthNorm === recordMonthNorm;
            const isFromCurrentPeriod = !forceHistorical && (
                frequency === 'DIARIO' || frequency === 'DIARIA'
                    ? isStrictCurrent
                    : granular
                        ? isSameReportableMonth  // cualquier semana/quincena del mes reportable
                        : (
                            isStrictCurrent || 
                            isSameMonthNorm ||
                            isCrossPeriodMatch ||
            ...
            const shouldShowInDashboard = isFromCurrentPeriod;
                    ...
                    setKpiData(prevData => {
                        ...
                        sortedGroups.forEach(group => {
                            ...
                            newData = newData.map(kpi => {
                                ...
                                const currentPeriodKey = getReportablePeriod(freq); // periodo anterior (semana/mes/quincena vencida)
                                const strictCurrentPeriod = getPeriodIndex(new Date(), freq); // periodo actual (no usar para dashboard)
                                // Periodo reportable en YYYY-MM
                                ...
                                // Para SEMANAL/QUINCENAL: aceptar cualquier periodo del mes reportable
                                // Para DIARIO: solo el día actual
                                // Para MENSUAL+: aceptar el mes reportable
                                const isFromCurrentPeriod = (freq === 'DIARIO' || freq === 'DIARIA')
                                    ? (group.periodKey === currentPeriodKey)
                                    : granular
                                        ? (groupMonthKey === currentMonthKey)  // cualquier semana/quincena del mes
                                        : (
                                            group.periodKey === currentPeriodKey ||
                                ...
                                // Para dashboard: mostrar solo si es del periodo reportable
                                const shouldShowInDashboard = isFromCurrentPeriod;
                                const brandValues = { ...(kpi.brandValues || {}) };
